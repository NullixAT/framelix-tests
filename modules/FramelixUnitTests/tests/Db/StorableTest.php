...
final class StorableTest extends TestCase
{
    ...
    {
        // enable system logs
        Config::set('systemEventLog[' . SystemEventLog::CATEGORY_STORABLE_CREATED . ']', true);
        Config::set('systemEventLog[' . SystemEventLog::CATEGORY_STORABLE_UPDATED . ']', true);
        Config::set('systemEventLog[' . SystemEventLog::CATEGORY_STORABLE_DELETED . ']', true);
        $db = Mysql::get('test');
        $this->setupDatabase();
        ...
        for ($i = 0; $i <= 50; $i++) {
            ...
            if (count($arr) >= 33) {
                ...
                $storableNewPrefetch->requiredIds = [
                    'normal' => array_keys($chunk),
                    'reverse' => array_keys($chunkRev),
                ];
    ...
    public function testFetch(): void
    {
        ...
        foreach ($storables as $storable) {
            ...
            // db value is always null or string as mysql driver doesnt support other values
            ...
            // some getter tests
            $this->assertNull($storable->getEditUrl());
        }
    ...
    public function testDatatypesPrefetchArray(): void
    {
        ...
        $prefetchLimit = Storable::getStorableSchemaProperty(
            TestStorablePrefetch::class,
            "otherReferenceArrayReducedPrefetch"
        )->prefetchLimit;
        // we have a reduced prefetch limit
        // a single getter will all prefetch max given number of storables
        // as we iterate over all storables, the prefetcher need a few step to prefetch all storables
    ...
    public function testMiscExceptions(): void
    {
        ...
        $this->assertFramelixErrorCode(ErrorCode::NOT_TYPEOF, $e);
        $e = null;
        ...
        $this->assertFramelixErrorCode(ErrorCode::STORABLE_ARRAY_DUPE_REFERENCES, $e);
        $e = null;
        try {
            TestStorable2::getByCondition(
                "otherReferenceArrayOptional.otherReferenceArrayOptional.notExist = {0}",
        ...
        $this->assertFramelixErrorCode(ErrorCode::STORABLE_SORT_DIRECTION_MISSING, $e);
        $e = null;
        try {
            $storable = new TestStorable2();
        ...
        $this->assertFramelixErrorCode(ErrorCode::NATIVE_CLONE, $e);
        $e = null;
        try {
            $storable = new TestStorable2();
            $storable->notExist = 1;
        ...
        $this->assertFramelixErrorCode(ErrorCode::STORABLE_PROPERTY_NOTEXIST, $e);
        $e = null;
        try {
            $storable = new TestStorable2();
        ...
        $this->assertFramelixErrorCode(ErrorCode::NOT_OPTIONAL, $e);
        $e = null;
        try {
            $storable = new TestStorable2();
            $storable->delete();
        ...
        $this->assertFramelixErrorCode(ErrorCode::STORABLE_NEW_DELETE, $e);
        $e = null;
        $storable = new TestStorableDeeper();
        $storable->store();
        try {
        ...
        $storable->delete(true);
        $this->assertFramelixErrorCode(ErrorCode::STORABLE_NOT_DELETABLE, $e);
    }
